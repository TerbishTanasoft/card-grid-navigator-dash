...
interface SystemCreateModalProps {
  isOpen: boolean
  onClose: () => void
  onCreate: (newSystem: any) => void
}

const systemTypes = ["Карт", "Коре", "Дотоод", "Дижитал"]

export function SystemCreateModal({ isOpen, onClose, onCreate }: SystemCreateModalProps) {
  ...
  const handleCreate = () => {
    if (formData.name && formData.description && formData.developer && formData.duration) {
      const newSystem = {
        ...formData,
        id: Date.now() // Энгийн ID үүсгэх
      }
      onCreate(newSystem)
      resetForm()
      onClose()
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3 pt-4 border-t">
            ...
            <Button 
              ...
              disabled={!formData.name || !formData.description || !formData.developer || !formData.duration}
            >
              Бүртгэх
            </Button>
          </div>
